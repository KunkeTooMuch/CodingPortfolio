...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // clear any prior results
    ...
    // declare variables
    std::string tempString = sql.c_str();                       // text style of sql
    std::string newsql;                                         
    std::smatch match;                                          // pattern matching with string
    std::regex numInjection(" or [1-9]+=[1-9]+;");              // 1-1 match of pattern
    std::regex wordInjection(" or '[a-zA-Z]+'='[a-zA-Z]+';");   // pattern will match 

    // searching for / detect sql patterns match: numeric
    // when detected, replace matching pattern with ';'
    // rerun with sanatized 'newsql'
    if (std::regex_search(tempString, match, numInjection)) {
        std::cout << "\nINJECTION DETECTED: Number Match has been found. Injected code removed.";
        newsql = std::regex_replace(tempString, numInjection, ";");
        return run_query(db, newsql, records);
    }
    // search / detect sql pattern match: words
    else if (std::regex_search(sql, match, wordInjection)) {
        std::cout << "\nINJECTION DETECTED: Word Match was found. Injected code removed.";
